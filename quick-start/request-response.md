...
DRF использует объект `Request`, расширяющий функционал стандартного `HttpRequest` и позволяющий более гибко разбирать запросы. Основной функционал `Request` в том, что он предоставляет атрибут `request.data`, который схож с `request.POST`, однако более полезен при работе с API.
...
Проверьте работу API из командной строки, как это делалось в [Уроке 1](serialization.md). Все работает одинаково, при этом мы получили улучшенную обработку ошибок в случае отправки неверных запросов.
