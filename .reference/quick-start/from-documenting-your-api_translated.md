...
Обязательно установите последнюю версию.
Библиотеки `pygments и` markdown` не являются обязательными, но рекомендуются.
...
Пользовательские действия на видах также могут быть задокументированы аналогичным образом, используя имена методов в качестве делимитеров или подключив документацию к методам сопоставления действий.
...
Следует ли считать схему *публичной *?
...
* `renderer_classes`: по умолчанию` none`.
Может использоваться для передачи пользовательских классов рендеринга в «SchemaView».
...
* `authentication_classes`: по умолчанию` api_settings.default_authentication_classes`.
Может использоваться для передачи пользовательских классов аутентификации в `schemaview '.
* `rescision_class`: по умолчанию` api_settings.default_permission_class`.
Может использоваться для передачи пользовательских классов разрешений в `schemaview '.
* `renderer_classes`: по умолчанию` none`.
Может использоваться для передачи пользовательских классов рендеринга в «SchemaView».
Если `none` `schemaview` будет настроен с` рендеристами `DocumentationRenderer` и` corejsonRenderer`, соответствующих форматам (по умолчанию) `html` и` corejson`.
...
Встроенная документация API включает в себя автоматически генерируемые образцы кода для каждой из доступных клиентских библиотек API.
...
Вы можете настроить эти образцы путем подкласса «DocumentRenderer», установив «языки» в список языков, которые вы хотите поддержать:
...
Для каждого языка вам необходимо предоставить шаблон «Intro», детализировать инструкции по установке и тому подобное, а также общий шаблон для выполнения запросов API, который может быть заполнен индивидуальными данными запроса.
См.
