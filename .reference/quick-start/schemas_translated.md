...
Вам нужно будет установить пакет `coreapi`, чтобы добавить поддержку схемы для Framework.
Вы, вероятно, также хотите установить `pyyaml`, чтобы вы могли представить схему в широко используемый формат OpenAPI на основе YAML.
...
См. Ниже [для более подробной информации] (#the-get_schema_view-shortcut) на настройке динамически сгенерированного представления схемы.
...
REST Framework использует [Core API] (https://www.coreapi.org/) для моделирования информации схемы в независимом формате представления.
Эта информация может затем быть представлена в различных форматах схемы или используется для генерации документации API.
...
При использовании Core API схема представлена как «документ», который является объектом контейнера верхнего уровня для информации об API.
Доступные взаимодействия API представлены с использованием объектов `link`.
Каждая ссылка включает в себя URL -адрес, метод HTTP и может включать список экземпляров «полевых», которые описывают любые параметры, которые могут быть приняты конечной точкой API.
Экземпляры `link` и` field 'могут также включать описания, которые позволяют отображать схему API в пользовательскую документацию.

Here's an example of an API description that includes a single `search` endpoint:

Вот пример описания API, который включает в себя одну конечную точку `search`:
...
Чтобы быть представленным в ответе HTTP, внутреннее представление должно быть представлено в фактических байтах, которые используются в ответе.

REST framework includes a few different renderers that you can use for encoding the API schema.

Структура REST включает в себя несколько различных рендеров, которые вы можете использовать для кодирования схемы API.
...
* `renderers.corejsonrenderer`-renders в [Core JSON] (https://www.coreapi.org/speciation/encoding/#core-json-encoding), формат, предназначенный для использования с клиентской библиотекой` coreapi.
...
Структура REST включает в себя класс рендеринга для обработки этого типа СМИ, который доступен как `рендереры.corejsonrenderer '.
...
Здесь стоит указать, что Core API также можно использовать для моделирования ответов на гипермедиа, которые представляют альтернативный стиль взаимодействия с схемами API.
...
С помощью схемы API весь доступный интерфейс представлен впереди как единственная конечная точка.
Ответы на отдельные конечные точки API затем обычно представляются в виде простых данных, без каких -либо дальнейших взаимодействий, содержащихся в каждом ответе.
...
С помощью гипермедиа клиенту вместо этого представлен документ, содержащий как данные, так и доступные взаимодействия.
Каждое взаимодействие приводит к новому документу, детализируя как текущее состояние, так и доступные взаимодействия.

Further information and support on building Hypermedia APIs with REST framework is planned for a future version.

Дополнительная информация и поддержка по созданию API Hypermedia с структурой REST запланированы для будущей версии.
...
Структура REST включает в себя функциональность для автоматической генерации схемы или позволяет вам явно указать ее.
...
Чтобы вручную указать схему, вы создаете основной API `Document ', аналогичный примеру выше.
...
`Schemagenerator 'обрабатывает список маршрутизированных шаблонов URL и составляет соответствующим образом структурированный документ API Core.

Basic usage is just to provide the title for your schema and call `get_schema()`:

Основное использование - это просто предоставить заголовок для вашей схемы и позвонить `get_schema ()`:
...
Это обеспечивает соответствующий объект API API Core `для представления, метода запроса и пути:
...
(При составлении схемы «Schemagenerator» вызывает `view.schema.get_link ()` Для каждого представления, разрешенный метод и путь.)
...
** ПРИМЕЧАНИЕ **: Для базовых подклассов `apiview` Interoscection по умолчанию по существу ограничено параметрами пути URL Kwarg.
Для подклассов `genericapiview`, которые включают все предоставленные классовые представления,` autoschema` попытается интроспективно сериализатор, лиц, лиц и фильтров, а также предоставить более богатые описания поля пути.
(Ключевые крючки вот соответствующие атрибуты и методы `genericapiview`,` get_serializer`, `pagination_class`,` filter_backends` и так далее.)
...
`` `Это допускает расширение для наиболее распространенного случая без подкласса.
...
...
schema = customschema ()
`` `Это обеспечивает полный контроль над интроспекцией.
* Создание `hanualSchema` на вашем взгляде, предоставляя основной API` fields` для представления явно:
`` `
от rest_framework.views import apiview
...
`` `Это позволяет вручную указать схему для некоторых представлений, сохраняя при этом автоматическую генерацию в другом месте.
...
** ПРИМЕЧАНИЕ **: Для получения полной информации о `schemagenerator` плюс« Autoschema` и `descriptors` manualSchema` см. [Ссылка на API ниже] (#api-reference).
...
Есть несколько разных способов добавить представление схемы в ваш API, в зависимости от именно того, что вам нужно.
...
Как только представление будет добавлено, вы сможете сделать запросы API, чтобы получить определение сгенерированной схемы.
...
Строка, представляющая путь импорта в конфункт URL, для которого вы хотите создать схему API.
...
Если вам нужно немного больше управления, чем дает вам ярлык `get_schema_view ()`, то вы можете использовать класс «Schemagenerator» непосредственно, чтобы автоматически генерировать экземпляр «Document» и вернуть его из представления.
...
Эта опция дает вам гибкость настройки конечной точки схемы с любым поведением, которое вы хотите.
Например, вы можете применить различные политики разрешения, дросселирования или аутентификации к конечной точке схемы.

Here's an example of using `SchemaGenerator` together with a view to return the schema.

Вот пример использования «Schemagerator» вместе с видом, чтобы вернуть схему.
...
Вы также можете обслуживать разные схемы различным пользователям, в зависимости от доступных им разрешений.
Этот подход может быть использован для обеспечения того, чтобы неавтотиционированные запросы были представлены с другой схемой для аутентифицированных запросов или для обеспечения того, чтобы различные части API были видимыми для разных пользователей в зависимости от их роли.
...
Чтобы представить схему с конечными точками, отфильтрованными разрешениями пользователей, вам необходимо передать аргумент `запроса` метод `get_schema ()`, как и так:
...
Альтернативой автоматическому подходу является явно указать схему API, объявив объект «документ» в вашей кодовой базе.
Это немного больше работы, но гарантирует, что вы полностью контролируете представление схемы.
...
Генерация схемы в структуре REST использует DocStrings для автоматической заполнения описаний в документе схемы.
...
Класс, который проходит список направленных шаблонов URL, запрашивает схему для каждого представления и сопоставляет результирующий документ Coreapi.
...
Это хороший момент для переопределения, если вы хотите изменить полученную структуру сгенерированной схемы, так как вы можете построить новый словарь с другой планировкой.
...
** `manual_fields` **: a` list` из экземпляров `coreapi.field`, которые будут добавлены в сгенерированные поля.
...
По умолчанию это основано на просмотре Docstring, как описано в разделе «Схемы как документация» выше.
...
Возвращает строку, чтобы указать кодирование для любого корпуса запроса при взаимодействии с данным представлением.
Например.
`'Приложение/json''.
Может вернуть пустую строку для представлений, которые не ожидают корпуса запроса.
...
Инкапсулирует логику для добавления или замены полей из списка на `field.name`.
...
Позволяет вручную предоставлять список экземпляров `coreapi.field` для схемы, а также дополнительное описание.
...
Эта документация дает краткий обзор компонентов в пакете Coreapi, которые используются для представления схемы API.
...
Чтобы обеспечить большую структуру для схемы, словарь «контент» может быть вложенным, как правило, на второй уровень.
Например:
...
Обратите внимание, что URL -адреса, которые поддерживают более одного метода HTTP, должны соответствовать одному `link` для каждого.
...
Логическое, указанное, если клиент должен включать значение, или если параметр может быть опущен.
...
Должен быть одной из следующих строк:
...
Действительно только если одно или несколько `location =" form "` fields, или одно поле `location =" body ", включено в` link`.
...
[drf-yasg] (https://github.com/axnsan12/drf-yasg/) генерирует [openapi] (https://openapis.org/) документы, подходящие для генерации кода-вложенные схемы, названные модели, тел ответов,
enum/pattern/min/max valdators, параметры формы и т. Д.
...
[DRF-Spectacular] (https://github.com/tfranzel/drf-spectacular/)-это инструмент [openapi 3] (https://openapis.org/) схемы с явным фокусом на расширяемости, настраиваемости и генерации клиентов.
Анкет
