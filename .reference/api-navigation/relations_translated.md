...
** ПРИМЕЧАНИЕ **: Это поле предназначено для объектов, которые отображают URL -адрес, который принимает один аргумент ключевого слова URL, как установлено с использованием аргументов `lookup_field` и` lookup_url_kwarg`.
...
Если вы используете [Стандартные классы маршрутизатора] (https://www.django-rest-framework.org/api-guide/routers#defaultrouter), это будет строка с форматом `<modelname> -detail`.
**требуется**.
* `Queryset` - QuerySet, используемый для поиска экземпляров модели при проверке ввода поля.
Отношения должны либо установить запрос явно, либо установить `read_only = true`.
* `Много ' - если применяется к ко многим отношениям, вы должны установить этот аргумент на` true'.
...
В отличие от ранее обсуждаемых * ссылок * на другую сущность, указанная объект вместо этого также может быть встроена или * вложена * в представление объекта, который относится к нему.
...
В редких случаях, когда ни один из существующих реляционных стилей не соответствует необходимому вам представлению, вы можете реализовать совершенно пользовательское реляционное поле, в котором точно описывается, как именно выходное представление должно генерироваться из экземпляра модели.
...
В некоторых случаях вам может потребоваться настроить поведение гиперссыщенного поля, чтобы представлять URL -адреса, которые требуют более одного поля поиска.

You can achieve this by overriding `HyperlinkedRelatedField`. There are two methods that may be overridden:

Вы можете достичь этого, переопределив `HyperlinkedRelatedField.
...
Может поднять `noreversematch`, если атрибуты` view_name` и `lookup_field` не настроены, чтобы правильно соответствовать конфу.
...
Если вы хотите поддержать поле с гиперссылкой для записи, то вы также захотите переопределить `get_object`, чтобы отобразить входящие URL -адреса обратно в объект, который они представляют.
Для гиперссылки только для чтения нет необходимости переопределять этот метод.

The return value of this method should the object that corresponds to the matched URL conf arguments.

Возвращаемое значение этого метода должен ли объект, который соответствует соответствующим аргументам Conf Conf.

May raise an `ObjectDoesNotExist` exception.

Может поднять исключение `ObjectDoesNotexist`.
...
Скажите, что у нас есть URL для объекта клиента, который принимает два аргумента ключевых слов, например, так:
...
В этом случае нам нужно было переопределить «HyperlinkedRelatedField», чтобы получить поведение, которое мы хотим:
...
Обратите внимание, что если вы хотите использовать этот стиль вместе с общими представлениями, вам также нужно переопределить `.get_object` в представлении, чтобы получить правильное поведение поиска.
...
Обычно мы рекомендуем плоский стиль для представлений API, где это возможно, но вложенный стиль URL также может быть разумным при использовании в меру.
...
Аргумент `Queryset` предназначен только для * поля отношений с записанием *, и в этом случае он используется для выполнения поиска экземпляра модели, который карты от примитивного пользовательского ввода, в экземпляр модели.
...
В версии 2.x класс сериализатора может * иногда * автоматически определять аргумент `Queryset` * Если используется класс Modelerializer '.

This behavior is now replaced with *always* using an explicit `queryset` argument for writable relational fields.

Это поведение теперь заменяется * всегда * с использованием явного аргумента `Queryset` для реляционных полей для записи.
...
Это уменьшает количество скрытой «магии», которое обеспечивает «моделиеализатор», делает поведение поля более ясным и гарантирует, что это тривиально перемещаться между использованием ярлыка «моделейализатор» или с использованием полностью явных классов «сериализатора».

## Customizing the HTML display

## Настройка HTML -дисплея
...
Встроенный метод модели «__str__` будет использоваться для генерации строковых представлений объектов, используемых для заполнения свойства».
Эти варианты используются для заполнения выбранных HTML -входов в API -файлах.
...
Чтобы предоставить настраиваемые представления для таких входов, переопределите `display_value ()` подкласса «Связанный поле».
Этот метод получит объект модели и должен вернуть строку, подходящую для его представления.
Например:
...
При визуализации в полевых полях API реляционные поля по умолчанию будут отображать максимум 1000 выбираемых элементов.
Если присутствует больше элементов, будет отображаться опция отключения с «более 1000 элементов…».
...
Такое поведение предназначено для предотвращения того, чтобы шаблон не мог представить в приемлемом временном промежутке из -за очень большого количества отображаемых отношений.

There are two keyword arguments you can use to control this behavior:

Есть два аргумента ключевых слов, которые вы можете использовать для управления этим поведением:
...
Установите, чтобы «нет», чтобы отключить любое ограничение.
...
Вы также можете управлять этими глобально, используя настройки `html_select_cutoff` и` html_select_cutoff_text`.
...
В тех случаях, когда применяется отсечение, вы можете вместо этого использовать простое поле ввода в форме HTML.
Вы можете сделать это, используя аргумент ключевого слова `style '.
Например:
...
Обратите внимание, что обратные отношения не включаются автоматически классами `modelerializer` и` HyperlinkedModelserializer '.
Чтобы включить обратные отношения, вы должны явно добавить их в список полей.
Например:
...
Обычно вы захотите убедиться, что вы установили соответствующий аргумент `indiend_name` на отношения, который вы можете использовать в качестве имени поля.
Например:
...
Если вы не установили связанное имя для обратного отношения, вам нужно использовать автоматически сгенерированное связанное имя в аргументе `Fields.
Например:
...
См. Документацию Django о [обратные отношения] (https://docs.djangoproject.com/en/stable/topics/db/queries/#following-realationships-backward) для получения более подробной информации.
...
Если вы хотите сериализовать общий внешний ключ, вам необходимо определить пользовательское поле, чтобы явно определить, как вы хотите сериализовать цели отношений.

For example, given the following model for a tag, which has a generic relationship with other arbitrary models:

Например, учитывая следующую модель для тега, которая имеет общую связь с другими произвольными моделями:
...
Мы могли бы определить пользовательское поле, которое можно использовать для сериализации метки экземпляров, используя тип каждого экземпляра, чтобы определить, как оно должно быть сериализовано:
...
Если вам нужна цель отношения, чтобы иметь вложенное представление, вы можете использовать необходимые сериализаторы внутри метода `.to_representation ()`:
...
Обратите внимание, что обратные общие клавиши, выраженные с использованием поля `genericrelation`, могут быть сериализованы с использованием регулярных типов реляционных поля, поскольку тип цели в отношениях всегда известен.
...
Для получения дополнительной информации см. [Документация Django по общим отношениям] (https://docs.djangoproject.com/en/stable/ref/contrib/contenttypes/#id1).

## ManyToManyFields with a Through Model

## manytomanyfields с моделью через

By default, relational fields that target a `ManyToManyField` with a `through` model specified are set to read-only.

По умолчанию реляционные поля, которые нацелены на `manytomanyfield` с указанной моделью« через », установлены только для чтения.
...
Если вы явно указали реляционное поле, указывающее на «ManyTomanyfield» с помощью модели, обязательно установите `read_only` на` true`.
...
Если вы хотите представить [дополнительные поля на модели через A (https://docs.djangoproject.com/en/stable/topics/db/models/#intermediary-manytomany), тогда вы можете сериализовать модель через [вложенную
Object] (https://www.django-rest-framework.org/api-guide/serializers/#dealing-with-nest-objects).
...
## Framework Framework общие отношения
...
В библиотеке [https://github.com/ian-foote/rest-framework-generic-realtions)