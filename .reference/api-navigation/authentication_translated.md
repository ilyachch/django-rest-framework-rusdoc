...
> AUTH должен быть подключенным.
>
>-Джейкоб Каплан-Мосс, ["РЕСЯТЬ ХУДЕНИЯ ПРАКТИКА"] (https://jacobian.org/writing/rest-worst-practices/)
...
Аутентификация - это механизм связывания входящего запроса с набором идентификационных учетных данных, таких как пользователь, из которого пришел запрос, или токен, с которым он был подписан.
[Разрешение] (разрешения. MD) и политики [Throttling] (Throttling.md) могут затем использовать эти учетные данные, чтобы определить, должен ли запрос быть разрешен.

REST framework provides several authentication schemes out of the box, and also allows you to implement custom schemes.

REST Framework предоставляет несколько схем аутентификации из коробки, а также позволяет реализовать пользовательские схемы.
...
Аутентификация всегда работает в самом начале представления, до того, как пройдут проверки разрешения и дросселя, и прежде чем любой другой код будет разрешен.

The `request.user` property will typically be set to an instance of the `contrib.auth` package's `User` class.

Свойство `request.user`, как правило, будет установлено на экземпляр класса« пользователь »пакета` contib.auth`.

The `request.auth` property is used for any additional authentication information, for example, it may be used to represent an authentication token that the request was signed with.

Свойство `request.auth` используется для любой дополнительной информации аутентификации, например, оно может использоваться для представления токена аутентификации, с которым был подписан запрос.

---

**Note:** Don't forget that **authentication by itself won't allow or disallow an incoming request**, it simply identifies the credentials that the request was made with.

** ПРИМЕЧАНИЕ: ** Не забывайте, что ** Аутентификация сама по себе не разрешает и не запрещает входящий запрос **, он просто определяет учетные данные, с которыми был сделан запрос.
...
Для получения информации о том, как настроить политику разрешений для вашего API, см. [Документация по разрешениям] (Permissions.md).
...
Схемы аутентификации всегда определяются как список классов.
Framework REST попытается аутентифицировать с каждым классом в списке и установит `request.user` и` request.auth` с использованием возвращаемого значения первого класса, который успешно аутентифицирует.

If no class authenticates, `request.user` will be set to an instance of `django.contrib.auth.models.AnonymousUser`, and `request.auth` will be set to `None`.

Если нет класса подлинности, `request.user` будет установлен на экземпляр` django.contrib.auth.models.anonymoususer`, и `request.auth` будет установлен на` none`.

The value of `request.user` and `request.auth` for unauthenticated requests can be modified using the `UNAUTHENTICATED_USER` and `UNAUTHENTICATED_TOKEN` settings.

Значение `request.user` и` request.auth` для несаутентированных запросов может быть изменено с помощью параметров `unautenticated_user` и` unauthenticated_token`.
...
Схемы аутентификации по умолчанию могут быть установлены по всему миру, используя настройку `default_authentication_classes`.
Например.
...
Вы также можете установить схему аутентификации на основе для каждого вида или на расстояние на расстоянии, используя представления на основе класса Apiview`.
...
Когда неавтотифицированный запрос будет отклонено в разрешении, существует два разных кода ошибок, которые могут быть подходящими.
...
* [Http 401 unauperized] (https://www.w3.org/protocols/rfc2616/rfc2616-sec10.html#sec10.4.2)
* [Http 403 разрешение отказано] (https://www.w3.org/protocols/rfc2616/rfc2616-sec10.html#sec10.4.4)

HTTP 401 responses must always include a `WWW-Authenticate` header, that instructs the client how to authenticate. HTTP 403 responses do not include the `WWW-Authenticate` header.

Ответы HTTP 401 всегда должны включать заголовок `www-authenticate`, который дает инструкции клиента, как аутентифицировать.
Ответы http 403 не включают заголовок `www-authenticate`.

The kind of response that will be used depends on the authentication scheme. Although multiple authentication schemes may be in use, only one scheme may be used to determine the type of response. **The first authentication class set on the view is used when determining the type of response**.

Вид ответа, который будет использоваться, зависит от схемы аутентификации.
Хотя могут использоваться несколько схем аутентификации, для определения типа ответа может использоваться только одна схема.
** Первый класс аутентификации, установленный в представлении, используется при определении типа ответа **.

Note that when a request may successfully authenticate, but still be denied permission to perform the request, in which case a `403 Permission Denied` response will always be used, regardless of the authentication scheme.

Обратите внимание, что когда запрос может успешно аутентифицировать, но все еще будет отказано в разрешении на выполнение запроса, и в этом случае ответ «403 разрешение отказано» всегда будет использоваться, независимо от схемы аутентификации.
...
Обратите внимание, что представление по умолчанию `eave_auth_token` явно использует запросы и ответы JSON, а не использование классов рендеринга по умолчанию и анализатора в ваших настройках.
...
Если вы хотите применить дросселирование, вам нужно переопределить класс View и включить их, используя атрибут `throttle_classes`.
...
В случае, если вы используете большую пользовательскую базу, мы рекомендуем вам обезьянуть класс `tokenadmin`, чтобы настроить его в соответствии с вашими потребностями, более конкретно, объявив поле« пользователь »как` raw_field`.
...
Ответы на неавентицентрированное разрешение приведет к ответу «HTTP 403».
...
Эта схема аутентификации позволяет вам делегировать аутентификацию на ваш веб -сервер, который устанавливает переменную среды remote_user`.
...
Чтобы использовать его, вы должны иметь `django.contrib.auth.backends.remoteuserbackend` (или подкласс) в своей настройке` authentication_backends`.
По умолчанию `remoteUserbackend` создает объекты пользователя для имен пользователей, которых еще не существует.
Чтобы изменить это и другое поведение, проконсультируйтесь с документацией [django] (https://docs.djangoproject.com/en/stable/howto/auth-remote-user/).
...
Например, обратитесь к документации вашего веб -сервера для получения информации о настройке метода аутентификации:
...
Это готовая к использованию реализации отдыха системы аутентификации Django.
