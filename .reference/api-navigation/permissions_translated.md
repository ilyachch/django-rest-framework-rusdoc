...
Немного менее строгий стиль разрешения будет заключаться в том, чтобы позволить полный доступ к аутентифицированным пользователям, но предоставление только для чтения доступ к несанкционированным пользователям.
...
*& mdash;
Http 401 несанкционированный ответ, с соответствующим заголовком `www-authenticate` будет возвращен.*
...
Если вы пишете свои собственные взгляды и хотите обеспечить разрешения на уровень объекта,
Или, если вы переопределяете метод `get_object` в общем представлении, вам необходимо явно вызвать метод` .check_object_permissions (запрос, obj) `на представлении в точке, в котором вы взяли объект.
...
Классы разрешений в `rest_framework.permissions` ** не ** Реализуйте
Методы, необходимые для проверки разрешений объекта.
...
Если вы хотите использовать предоставленные классы разрешений, чтобы проверить объект
разрешения, ** вы должны ** подкласс их и реализовать
`has_object_permission ()` Метод, описанный в [*Пользовательский
Раздел разрешения*] (#Custom-Permissions) Раздел (ниже).
...
Вы также можете установить политику аутентификации на основе или на основе на расстояние,
...
Если вам нужно проверить, является ли запрос операцией чтения или операцией записи, вам следует проверить метод запроса на постоянный `safe_methods`, который является кортежом, содержащим` 'get' ', `' опции 'и` `head
'.
Например:
...
<br>
** Методы `get _*()` имеют доступ к текущему представлению и могут возвращать различные экземпляры сериализатора или запроса на основе запроса или действия.
