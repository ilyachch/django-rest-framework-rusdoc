...
Тем не менее, иногда вы захотите поместить свою логику проверки в компоненты многократного использования, чтобы ее можно было легко повторно использовать на всей вашей кодовой базе.
...
Если вы хотите, чтобы поле даты подлежит записи, единственное, что стоит отметить, это то, что вы должны убедиться, что оно всегда доступно во входных данных, либо путем установки аргумента «по умолчанию», либо установив `required = true`.
...
Есть несколько неоднозначных случаев, когда вам нужно вместо этого, вместо этого выяснить проверку явно, а не полагаться на классы сериализатора по умолчанию, которые генерирует `modelerializer».
...
По умолчанию «уникальная вместе» проверка обеспечивает соблюдение того, что все поля должны быть `обязательно = true`.
В некоторых случаях вы можете явно применить `required = false` к одному из полей, и в этом случае желаемое поведение проверки является неоднозначным.
...
В этом случае вам обычно нужно исключить валидатор из класса сериализатора, а вместо этого явно написать любую логику проверки, либо в методе `.validate ()`, либо в представлении.
...
При применении обновления к существующему экземпляру валидаторы уникальности исключат текущий экземпляр из проверки уникальности.
Текущий экземпляр доступен в контексте проверки уникальности, поскольку он существует в качестве атрибута на сериализаторе, изначально был пройден с помощью `encement = ...` при создании сериализатора.
...
В случае операций обновления на * вложенных * сериализаторах нет никакого способа применять это исключение, поскольку экземпляр недоступен.
...
Опять же, вы, вероятно, захотите явно удалить валидатор из класса Serializer и явно написать код для ограничения проверки в методе `.validate ()` или в представлении.
...
Если вы не уверены точно, какое поведение будет генерировать класс `moderializer`, это обычно является хорошей идеей для запуска` Manage.py Shell
автоматически генерирует для вас.
...
Также имейте в виду, что при сложных случаях часто может быть лучше явно определить ваши классы сериализатора, а не полагаться на поведение моделейализатора по умолчанию.
Это включает в себя немного больше кода, но гарантирует, что результирующее поведение является более прозрачным.
...
Это задокументировано в [Serializer Docs] (https://www.django-rest-framework.org/api-guide/serializers/#field-salidation)
...
В некоторых расширенных случаях вы можете захотеть пройти валидатор в поле сериализатора, с которым он используется в качестве дополнительного контекста.
Вы можете сделать это, установив атрибут `trany_context = true` на валидаторе.
Затем метод `__CALL__ будет вызван с помощью` serializer_field` или `serializer` в качестве дополнительного аргумента.
