...
* По соглашению вы должны импортировать модуль `serializers` и использовать полностью сформированный<!-- не уверен насчет формулировки --> `ValidationError`, чтобы отличить его от встроенной ошибки валидации Django. Например. `raise serializers.ValidationError('Это поле должно быть целочисленным значением.')`.
