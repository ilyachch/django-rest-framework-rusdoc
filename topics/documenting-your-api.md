...
В REST Framework рекомендуется использовать сторонние пакеты для генерации и представления схем OpenAPI, поскольку они предлагают больше возможностей и гибкости, чем встроенная (устаревшая) реализация.

### drf-spectacular

[drf-spectacular](https://github.com/tfranzel/drf-spectacular/) — это библиотека для генерации схем [OpenAPI 3](https://openapis.org/), в которой особое внимание уделяется расширяемости, настраиваемости и генерации клиентских интерфейсов. Это рекомендуемый способ генерации и представления схем OpenAPI.

Библиотека нацелена на извлечение максимально возможного объема информации о схеме, при этом предоставляя декораторы и расширения для удобной настройки. Предусмотрена явная поддержка [swagger-codegen](https://swagger.io/), [SwaggerUI](https://swagger.io/tools/swagger-ui/) и [Redoc](https://github.com/Rebilly/ReDoc), i18n, управления версиями, аутентификации, полиморфизма (динамические запросы и ответы), параметров запроса/пути/заголовка, документации и многого другого. Также из коробки поддерживаются несколько популярных плагинов для DRF.
...
**Уведомление об устаревании**: встроенная поддержка генерации схем OpenAPI в REST Framework больше не рекомендуется к использованию; вместо этого следует использовать сторонние пакеты, предоставляющие эту функциональность. В качестве альтернативы мы рекомендуем использовать **drf-spectacular**.
...
Расширенные возможности использования см. в [документации Swagger UI](https://swagger.io/tools/swagger-ui/).
