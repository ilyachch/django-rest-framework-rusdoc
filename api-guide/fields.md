...
Примечания относительно распространения значений по умолчанию от модели к сериализатору:

- Все значения по умолчанию из модели будут переданы по умолчанию в сериализатор и метод `options`.
- Если значение `default` является вызываемым, то оно будет передаваться и выполняться каждый раз в сериализаторе, но не в методе options.
- Если значение для данного поля не задано, то значение по умолчанию будет присутствовать в сериализаторе и доступно в методах сериализатора. Заданная валидация для данного поля будет выполнена по значению по умолчанию, так как это поле будет присутствовать в сериализаторе.
...
Обратите внимание, что в этом примере не предусмотрена валидация. Отчасти по этой причине в
реальном проекте вложенность координат может быть лучше обработана с помощью вложенного сериализатора
