...
Замечания по поводу распространения значений по умолчанию от модели к сериализатору:

- Все значения по умолчанию из модели будут передаваться как значения по умолчанию в сериализатор и метод options.
- Если значение по умолчанию является вызываемым, то оно будет передаваться и выполняться каждый раз в сериализаторе, но не в методе options.
- Если значение для данного поля не задано, то значение по умолчанию будет присутствовать в сериализаторе и доступно в методах сериализатора. Заданная валидация для данного поля будет оцениваться по значению по умолчанию, так как это поле будет присутствовать в сериализаторе.
...
* ``ormat`: Определяет формат представления значения uuid
...
* `coerce_to_string` Устанавливается в `True`, если для представления должны быть возвращены строковые значения, или `False`, если должны быть возвращены объекты `Decimal`. По умолчанию имеет то же значение, что и ключ настройки `COERCE_DECIMAL_TO_STRING`, который будет `True`, если его не переопределить. Если сериализатор возвращает объекты `Decimal`, то окончательный формат вывода будет определяться рендерером. Обратите внимание, что установка `localize` заставит значение быть `True`.
...
* `normalize_output` Нормализует десятичное значение при сериализации. Это удалит все нули и изменит точность значения до минимально необходимой, чтобы можно было представить значение без потери данных. По умолчанию `False`.
...
* `format` - строка, представляющая формат вывода. Если она не указана, то по умолчанию используется то же значение, что и ключ настройки `TIME_FORMAT`, который будет `'iso-8601'`, если он не установлен. Установка в строку формата указывает, что возвращаемые значения `to_representation` должны быть принудительно выведены в строковый формат. Строки формата описаны ниже. Установка этого значения в `None` указывает, что объекты Python `time` должны быть возвращены `to_representation`. В этом случае кодировка времени будет определяться рендерером.
...
* Если наше поле не предназначено только для чтения, `to_internal_value` должно возвращать дикту, подходящую для обновления нашего целевого объекта. При `source='*'`, возврат из `to_internal_value` будет обновлять корневой словарь данных, а не один ключ.
