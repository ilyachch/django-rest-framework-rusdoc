...
DRF позволяет объединять логику для набора связанных представлений в одном классе, называемом `ViewSet`. В других фреймворках вы также можете встретить концептуально схожие реализации с такими названиями, как "Resources" или "Controllers".

Класс `ViewSet` - это просто **тип классового представления (View), который не предоставляет никаких обработчиков методов**, таких как `.get()` или `.post()`, а вместо этого предоставляет действия, такие как `.list()` и `.create()`.
...
Как правило, вместо явной регистрации представлений в наборе представлений в urlconf, вы регистрируете набор представлений с помощью класса маршрутизатора, который автоматически определяет urlconf за вас.
...
**Предупреждение**: Не используйте `.as_view()` с методами `@action`. Это обходит настройки маршрутизатора и может привести к игнорированию таких настроек действия, как `permission_classes`. Для действий используйте `DefaultRouter`.
...
**Предупреждение**: При регистрации наборов представлений не включайте конечный слэш в префикс (например, используйте `r'users'`, а не `r'users/'`). В отличие от стандартных шаблонов URL в Django, маршрутизаторы DRF автоматически добавляют слэши в соответствии с настройками конечного слэша.
