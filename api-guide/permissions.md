...
**Примечание**: при составлении разрешений поддерживаются операторы `&` (и), `|` (или) и `~` (не), а также допускается использование скобок `(` `)` для группировки выражений.

Операторы следуют тем же правилам приоритета и ассоциативности, что и стандартные логические операторы (наивысший приоритет у `~`, затем `&`, затем `|`).
