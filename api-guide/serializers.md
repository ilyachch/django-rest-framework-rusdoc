...
#### field_class и API field_kwargs
...
Класс `ListSerializer` обеспечивает возможность одновременной сериализации и проверки нескольких объектов. Как правило, вам не понадобится использовать `ListSerializer` напрямую; вместо этого достаточно просто передать параметр `many=True` при создании экземпляра сериализатора.

При создании экземпляра сериализатора с параметром `many=True` будет создан экземпляр `ListSerializer`. Затем класс сериализатора становится дочерним по отношению к родительскому `ListSerializer`.

Следующий аргумент также можно передать полю `ListSerializer` или сериализатору, которому передано `many=True`:

- `allow_empty`: по умолчанию это значение равно `True`, но его можно установить в `False`, если вы хотите запретить пустые списки в качестве допустимого ввода.

- `max_length`: по умолчанию это `None`, но может быть установлено в положительное целое число, если вы хотите проверить, что список содержит не более этого числа элементов.

- `min_length`: по умолчанию это `None`, но может быть установлено в положительное целое число, если вы хотите проверить, что список содержит не менее этого числа элементов.
...
Пакет [drf-pydantic](https://github.com/georgebv/drf-pydantic) позволяет использовать Pydantic с Django REST Framework для проверки данных и (де)сериализации. Если вы разрабатываете API на основе DRF и используете Pydantic для проверки данных, этот пакет обеспечивает беспроблемную интеграцию между обеими библиотеками.
