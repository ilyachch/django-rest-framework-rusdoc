...
В подклассах `GenericAPIView` также можно задать атрибут `pagination_class`, чтобы выбрать `PageNumberPagination` для каждого конкретного представления.

По умолчанию в качестве имени параметра запроса, используемого для пагинации, используется `page`. Это можно настроить, создав подкласс `PageNumberPagination` и переопределив атрибут `page_query_param`.
...
С такой конфигурацией клиенты будут запрашивать страницы, используя `?p=2` вместо `?page=2`.
